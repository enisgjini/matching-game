...
namespace MatchingGame
{
    public partial class MainForm : Form
    {
        ...
        private void CheckForWinner()
        {
            ...
            if (bestScore == 0)
            {
                text = $"Ju mori {currentScore} tentativa për të përfunduar. Vazhdo kështu!";
            }
            else if (bestScore < currentScore)
            {
                text = $"Ju mori {currentScore - bestScore} më shumë tentativa se më e mira juaj e mëparshme. Përpiquni më shumë!";
            }
            else
            {
                text = $"Ju vendosni një më të mirë të re vetëm me {currentScore} tentativa!";
            }
            MessageBox.Show(text, "Përgëzime!");
            StartNewGame();
        }
    }
}